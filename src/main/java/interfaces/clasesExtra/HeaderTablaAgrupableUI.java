...
public class HeaderTablaAgrupableUI extends BasicTableHeaderUI {

    protected HeaderTablaAgrupable getHeader() {
        return (HeaderTablaAgrupable) header;
    }
    ...
    public void paint(Graphics g, JComponent c) {
        ...
        Map<ColumnaAgrupada, Rectangle> groupSizeMap = new HashMap<ColumnaAgrupada, Rectangle>();

        for (Enumeration<TableColumn> enumeration = header.getColumnModel().getColumns(); enumeration.hasMoreElements();) {
            cellRect.height = size.height;
            cellRect.y = 0;
            TableColumn aColumn = enumeration.nextElement();
            List<ColumnaAgrupada> groups = getHeader().getColumnGroups(aColumn);
            int groupHeight = 0;
    ...
    private void paintCell(Graphics g, Rectangle cellRect, int columnIndex) {
        ...
        c.setBackground(Color.decode("#424242"));
        c.setForeground(Color.WHITE);

        rendererPane.paintComponent(g, c, header, cellRect.x, cellRect.y, cellRect.width, cellRect.height, true);
    }

    private void paintCell(Graphics g, Rectangle cellRect, ColumnaAgrupada cGroup) {
